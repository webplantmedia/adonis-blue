...
function angie_makes_design_posted_on() {
	...
	$posted_on = sprintf(
		...
		esc_html_x( 'Posted on %s', 'post date', 'angie-makes-design' ),
...
function angie_makes_design_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list ) {
			...
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'angie-makes-design' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}
